...
1. **`Backend` (Spring Boot 3 + Java 17+)**
   - Roda embarcado na porta `8081`. (Alterada para evitar conflitos de segurança)
...
2. **`Frontend` (React + Vite)**
   ...
   - Adota **Glassmorphism**, contrastes em Dark Mode e componentes dinâmicos para a navegação suave do carrinho e checkout da venda e painel tático.

## 🖥️ Como inicializar a Solução

**Passo 1: Levantar a API (Backend)**
Dentro da pasta do backend `sist-vendas-v2-rep-mem-incompleto` rode:
```bash
./mvnw clean spring-boot:run
```

**Passo 2: Levantar o Portal (Frontend)**
Em outro terminal paralelo, desça até a pasta `/frontend` e baixe as dependências instantâneas:
```bash
npm install
npm run dev
```
